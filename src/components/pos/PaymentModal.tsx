...
export default function PaymentModal({ isOpen, onClose, defaultMethod = 'cash' }: PaymentModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 transition-opacity">
      <div className="bg-white dark:bg-pos-dark-card w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden border border-gray-100 dark:border-pos-dark-border">
        
        {/* Header */}
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-pos-dark-border bg-gray-50 dark:bg-pos-dark-panel">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Complete Payment</h2>
          <button 
            onClick={onClose}
            disabled={isProcessing}
            className="p-2 text-gray-500 hover:text-gray-900 dark:hover:text-white rounded-full hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors disabled:opacity-50"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        {/* Tabs */}
        <div className="flex border-b border-gray-100 dark:border-pos-dark-border">
          {[
            { id: 'cash', label: 'Cash' },
            ...
            <button
              ...
              className={`flex-1 py-4 text-sm font-semibold transition-colors border-b-2 ${
                activeTab === tab.id
                  ? 'border-pos-accent text-pos-accent dark:text-pos-accent bg-pos-accent/5'
                  : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 hover:bg-gray-50 dark:hover:bg-pos-dark-panel'
              } disabled:opacity-50`}
            >
              {tab.label}
            </button>
        ...
        {/* Content */}
        <div className="p-6">
          <div className="mb-6 flex justify-between items-end border-b border-gray-100 dark:border-pos-dark-border pb-4">
            <span className="text-gray-500 dark:text-gray-400 font-medium">Total Due</span>
          ...
          <div className="min-h-[160px]">
            ...
            {activeTab === 'card' && (
              <div className="flex flex-col items-center justify-center h-32 space-y-3">
                <div className="animate-pulse p-4 rounded-full bg-gray-100 dark:bg-pos-dark-panel">
        ...
        {/* Footer */}
        <div className="p-5 border-t border-gray-100 dark:border-pos-dark-border bg-gray-50 dark:bg-pos-dark-panel">
          <button
            onClick={handleTransaction}
            disabled={isProcessing || total === 0}
            className="w-full py-4 rounded-xl bg-pos-accent text-pos-accent-text font-bold text-lg shadow-sm hover:brightness-95 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isProcessing ? (
              <>
