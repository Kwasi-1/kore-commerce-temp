...
export default function ProcessPayrollModal({
  ...
  // Re-sync if initialSelectedId or profiles change when opening if no draft exists
  useEffect(() => {
    if (isOpen && profiles.length > 0 && itemStates.length === 0) {
      setActiveStepKey('1');
      setItemStates(
        profiles.map((p) => ({
          profile_id: p.id,
  ...
  const isFormDirty = itemStates.some(
    (item) => item.adjustment !== 0 || item.note || !item.selected
  );
  ...
  return (
    <>
      <CustomModal
        ...
        header={
          <div className="pt-2 px-2 border-b border-border pb-4 flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold">Run Regular Payroll</h2>
              <p className="text-sm text-muted-foreground font-normal">
                Review gross pay, custom adjustments, and disburse staff salaries.
              </p>
            </div>
