    ...
    <div className="flex h-screen w-full transition-colors duration-200 overflow-hidden">
      ...
      <div className="flex w-full h-full bg-dark-sidebar dark:bg-sidebar shadow-2xl overflow-hidden ring-1 ring-black/5">
        ...
        <div className={`flex flex-col flex-1 overflow-hidden transition-all duration-300 p-0 md:py-2 md:pr-2 ${isSidebarCollapsed ? 'md:pl-0' : 'md:pl-0'}`}>
          <div className={`flex flex-col flex-1 overflow-hidden bg-background shadow-inner borde border-black/5 relative transition-all duration-300 scrollbar-hide ${showBottomNav ? 'pb-16 md:pb-0' : 'pb-0'} ${isSidebarCollapsed ? 'rounded-none  md:rounded-[1.25rem] lg:rounded-[1.15rem]' : 'rounded-none md:rounded-[1.0rem]'}`}>
            <main className="flex-1 overflow-y-auto px-4 md:px-5 py-2 md:py-0 scrollbar-hide">
              <Outlet />
            </main>
          </div>
        </div>
