      ...
      payment_info: (
        <div className="flex flex-col text-xs">
          {p.payment_method === 'cash' ? (
            ...
            <>
              <span className="font-semibold text-foreground">{p.bank_or_momo_name || 'Mobile Money'}</span>
              <span className="text-muted-foreground font-mono text-[11px]">{p.account_number || 'No phone set'}</span>
            </>
          ) : (
            <>
              <span className="font-semibold text-foreground">{p.bank_or_momo_name || 'Bank Transfer'}</span>
              <span className="text-muted-foreground font-mono text-[11px]">{p.account_number || 'No account set'}</span>
            </>
          )}
        </div>
      ),
